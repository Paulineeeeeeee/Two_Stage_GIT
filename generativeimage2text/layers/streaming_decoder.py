...
class CaptioningModel(nn.Module):
    def __init__(
        ...
        if context_not_share_embedding:
            self.text_encoder = self.textual.embedding
    ...
    def preprocess_features(self, image, text = None, per_frame = True):
        """
        Processes image and optionally text features.

        Args:
            image (torch.Tensor): Input image tensor (length, channels, height, width).
            text (torch.Tensor, optional): Input text tensor (length, q). Defaults to None.
                                           Format depends on self.paddle logic.
            per_frame (bool, optional): Strategy for image feature selection when
                                        downsampling is needed and not in paddle mode.
                                        Defaults to True.

        Returns:
            torch.Tensor: Combined image and text features, or just processed
                          image features. Shape varies based on paddle flag
                          and input lengths.
        """
        ...
        with torch.no_grad():
            ...
            features = self.image_encoder(image) # Shape: (n_frames, token_size, d) or similar
            n_frames = features.shape[0]
            # Assuming token_size is the number of spatial/patch tokens per frame *after* image_encoder
            token_size = features.shape[1] # e.g., 197 for ViT-B/16

            # Add temporal embeddings if configured
            if self.num_image_with_embedding and self.img_temperal_embedding:
                # The original code samples temporal embeddings based on num_image_with_embedding
                # and interpolates to the actual n_frames length. 
                temp_emb_tensor = torch.stack(list(self.img_temperal_embedding), dim=0)  # shape: (num_img_emb, 1, 1, d) if embeddings are (1,1,d)
                temp_emb_tensor = temp_emb_tensor.squeeze(1).squeeze(1) # shape: (num_img_emb, d)
                temp_emb_tensor = temp_emb_tensor.permute(1, 0).unsqueeze(0) # shape: (1, d, num_img_emb)
                # Interpolate to n_frames along the time dimension (dim 2)
                temp_emb_tensor = F.interpolate(temp_emb_tensor, size=n_frames, mode='linear', align_corners=False)  # shape: (1, d, n_frames)
                temp_emb_tensor = temp_emb_tensor.squeeze(0).permute(1, 0) # shape: (n_frames, d)
                temp_emb_tensor = temp_emb_tensor.unsqueeze(1) # shape: (n_frames, 1, d) -> broadcastable

                features = features + temp_emb_tensor # Add temporal embedding: (n_frames, token_size, d) + (n_frames, 1, d)

            # Reshape/Pool image features
            # The default case (pooling_images is None) reshapes.
            if self.pooling_images is None:
                visual_features = features.reshape(-1, features.shape[-1])
            elif self.pooling_images == 'avg':
                visual_features = features.mean(dim=1) # shape (n_frames, d)
                token_size = 1 # Update token_size for subsequent calculations if needed
            else:
                raise NotImplementedError(f"Pooling method {self.pooling_images} not implemented.")

            # visual_features shape is now (TotalVisualTokens, d), where TotalVisualTokens
            # is either n_frames * original_token_size (if pooling=None) or n_frames (if pooling='avg')

            processed_features = None
            features_valid = None # Initialize features_valid for this call

            if self.paddle:
                # Logic for multimodal case with text and valid masks

                # --- Process Visual Features (within Paddle logic) ---
                # In the original paddle logic, visual features are selected/sampled
                # based on num_image_with_embedding * token_size *before* concatenating text.
                target_visual_tokens = self.num_image_with_embedding * token_size # Use the possibly updated token_size if pooling reduced it

                if len(visual_features) > target_visual_tokens:
                    # Downsample visual features to the target number of tokens
                    vis_i = self.get_streaming_features(visual_features, target_visual_tokens)
                else:
                     # Keep all visual features if already within the limit
                    vis_i = visual_features

                # Create valid mask for selected visual features
                valid_vis_i = convert2valid(vis_i.unsqueeze(0).shape[:2]) # Assuming convert2valid takes shape (batch, len)

                # --- Process Text Features ---
                text = text.to("cuda")
                context_embedding_layer = self.text_encoder # Assuming this is a callable like nn.Embedding

                cls_id = self.tokenizer.cls_token_id
                sep_id = self.tokenizer.sep_token_id

                if n_frames > self.num_image_with_embedding:
                    # When more frames than num_image_with_embedding: concatenate text tokens across frames
                    image_token_count = target_visual_tokens # Number of visual tokens
                    # The original code calculates text_tokens based on num_image_with_embedding
                    # text_tokens = self.num_image_with_embedding * 40 # Example fixed text tokens per image embedding location?
                    # Let's interpret this as the total desired text tokens when n_frames > num_image_with_embedding
                    total_desired_text_tokens = self.num_image_with_embedding * 40 # Assuming 40 text tokens per effective image location

                    # Filter and concatenate text tokens from all frames
                    all_text_tokens = []
                    for idx in range(n_frames):
                        frame_text_token = text[idx] # text[idx] is shape (q,)
                        # Remove special tokens and padding (assuming 0 is padding)
                        filtered_text_token = frame_text_token[(frame_text_token != cls_id) & (frame_text_token != sep_id) & (frame_text_token != 0)]
                        all_text_tokens.append(filtered_text_token)

                    # Concatenate all valid text tokens
                    device = all_text_tokens[0].device if all_text_tokens else text.device # Handle case with no text
                    valid_tokens = torch.cat(all_text_tokens, dim=0) # shape (TotalValidTextLength,)
                    total_valid_text_length = valid_tokens.shape[0]

                    # Limit total valid text tokens if necessary (original code limit was 800?)
                    # And then sample down to total_desired_text_tokens - 2 (for CLS/SEP)
                    text_token_limit = total_desired_text_tokens - 2 # Reserve 2 tokens for CLS and SEP

                    if total_valid_text_length > 0: # Only process if there's actual text
                        # Apply limit before embedding if needed (original code limited to 800 before embedding)
                        if total_valid_text_length > 800:
                            valid_tokens = valid_tokens[:800]
                            total_valid_text_length = valid_tokens.shape[0] # Update length

                        # Embed the valid tokens
                        context_feat_raw = context_embedding_layer(valid_tokens.unsqueeze(0))  # shape: (1, CurrentValidTextLength, d)

                        # Apply streaming features if still too long after initial limit
                        if total_valid_text_length > text_token_limit:
                            context_feat = self.get_streaming_features(context_feat_raw.squeeze(0), text_token_limit) # shape: (text_token_limit, d)
                            context_feat = context_feat.unsqueeze(0) # shape: (1, text_token_limit, d)
                        else:
                            context_feat = context_feat_raw # shape: (1, CurrentValidTextLength, d)

                        # Add CLS and SEP tokens
                        cls_tensor = torch.tensor([cls_id], device=device)
                        sep_tensor = torch.tensor([sep_id], device=device)
                        cls_feat = context_embedding_layer(cls_tensor.unsqueeze(0))  # shape: (1, 1, d)
                        sep_feat = context_embedding_layer(sep_tensor.unsqueeze(0))  # shape: (1, 1, d)

                        # Concatenate CLS, text features, SEP
                        context_feat = torch.cat([cls_feat, context_feat, sep_feat], dim=1) # shape: (1, EffectiveTextTokens, d)
                        effective_text_length = context_feat.shape[1] # Includes CLS/SEP

                        # Pad text features to reach total_desired_text_tokens if needed
                        if effective_text_length < total_desired_text_tokens:
                            pad_size = total_desired_text_tokens - effective_text_length
                            pad_tensor = torch.zeros((1, pad_size, context_feat.size(-1)), device=device)
                            context_feat = torch.cat([context_feat, pad_tensor], dim=1)

                        # Create valid mask for the padded text features
                        valid_text_i = convert2valid(context_feat.shape[:2], torch.tensor([effective_text_length], device=device))

                    else: # Case where there is no text after filtering
                        # Create a placeholder for text features with 0 length and a dummy valid mask
                        context_feat = torch.empty((1, 0, vis_i.size(-1)), device=device)
                        valid_text_i = convert2valid(context_feat.shape[:2], torch.tensor([0], device=device))


                else:
                    # When fewer or equal frames than num_image_with_embedding: Process text per frame
                    sample_chunks = []
                    sample_valids = []

                    for i in range(n_frames):
                        frame_text_tokens = text[i]  # shape: (q,)

                        # Determine valid length based on last SEP or total length
                        sep_positions = torch.where(frame_text_tokens == sep_id)[0]
                        if len(sep_positions) > 0:
                            text_length = sep_positions[-1].item() + 1
                        else:
                            # If no SEP, consider all tokens up to the first padding (0) as valid
                            non_padding_tokens = torch.where(frame_text_tokens != 0)[0]
                            if len(non_padding_tokens) > 0:
                                text_length = non_padding_tokens[-1].item() + 1
                            else:
                                text_length = 0 # No text tokens

                        # Embed frame text (only valid part might be needed depending on context_embedding_layer)
                        # Let's embed the full tensor and mask later if needed, or pass only valid tokens
                        # Original code embeds the full text_tokens tensor: context_embedding(text_tokens.unsqueeze(0))
                        context_feat = context_embedding_layer(frame_text_tokens.unsqueeze(0)) # shape: (1, q, d)

                        # Create valid mask for this frame's text
                        # The mask should cover tokens up to text_length
                        valid_text_i_frame = convert2valid(context_feat.shape[:2], torch.tensor([text_length], device=context_feat.device)) # shape (1, q)

                        sample_chunks.append(context_feat) # shape (1, q, d)
                        sample_valids.append(valid_text_i_frame) # shape (1, q)

                    # Concatenate text features and valid masks from all frames
                    if sample_chunks: # Handle case with no frames/text
                        context_feat = torch.cat(sample_chunks, dim=1) # shape: (1, n_frames * q, d)
                        valid_text_i = torch.cat(sample_valids, dim=1) # shape: (1, n_frames * q)
                    else:
                        # Case with no frames or no text
                        context_feat = torch.empty((1, 0, vis_i.size(-1)), device=device)
                        valid_text_i = convert2valid(context_feat.shape[:2], torch.tensor([0], device=device))


                # --- Final Concatenation for Paddle ---
                # Concatenate visual and text features
                processed_features = torch.cat([vis_i, context_feat.squeeze(0)], dim=0) # vis_i shape (TotalVisualTokens, d), context_feat shape (1, TotalTextTokens, d)
                # Concatenate valid masks
                features_valid = torch.cat([valid_vis_i, valid_text_i], dim=1)
                

            else:
                # Logic for image-only case (or non-paddle multimodal if that existed)

                # Apply visual feature selection/downsampling logic from process_image_features
                # This logic is slightly different: it can do per-frame selection
                # if per_frame is True and n_frames > num_image_with_embedding.

                target_visual_tokens = self.num_image_with_embedding * token_size # Use the possibly updated token_size

                if self.num_image_with_embedding and len(visual_features) > target_visual_tokens:
                    # Selection/Downsampling needed if we have num_image_with_embedding set
                    # and the current number of visual tokens exceeds the target.

                    if per_frame and n_frames > self.num_image_with_embedding:
                        # Strategy 1: Select features per frame
                        # Calculate how many tokens to take from each frame
                        base_tokens_per_frame = target_visual_tokens // n_frames
                        remainder = target_visual_tokens % n_frames

                        new_features_list = []
                        current_idx_in_visual_features = 0
                        # Loop through original frames (implicitly via slicing visual_features)
                        for idx in range(n_frames):
                            # Determine tokens to extract for this frame
                            extra = 1 if idx >= n_frames - remainder else 0
                            tokens_to_extract = base_tokens_per_frame + extra

                            # Extract features corresponding to the current frame from the reshaped visual_features
                            # visual_features shape is (n_frames * original_token_size, d) if pooling=None
                            # or (n_frames, d) if pooling='avg' (and token_size became 1)
                            # This slicing assumes visual_features maintains frame order implicitly.
                            # If pooling was 'avg', token_size is 1, and this slicing works differently.
                            # Let's assume visual_features shape is (TotalOriginalTokens, d) where TotalOriginalTokens = n_frames * original_token_size
                            original_tokens_this_frame = features.shape[1] if self.pooling_images is None else 1 # Use original token_size before reshape/pool if pooling is None
                            frame_features_slice = visual_features[current_idx_in_visual_features : current_idx_in_visual_features + original_tokens_this_frame]

                            # Apply get_streaming_features to the current frame's slice
                            selected_frame_features = self.get_streaming_features(frame_features_slice, tokens_to_extract)

                            new_features_list.append(selected_frame_features)
                            current_idx_in_visual_features += original_tokens_this_frame # Move to the start of the next frame's slice

                        # Concatenate all selected frame features
                        processed_features = torch.cat(new_features_list, dim=0) # shape: (target_visual_tokens, d)

                    else:
                        # Strategy 2: Select features from the total pool
                        # This handles cases where per_frame is False, or n_frames <= num_image_with_embedding
                        # but still need to downsample because len(visual_features) > target_visual_tokens
                        processed_features = self.get_streaming_features(visual_features, target_visual_tokens)
                else:
                    # No downsampling needed, use all available visual features
                    processed_features = visual_features # shape: (len(visual_features), d)

                # For the non-paddle case, features_valid remains None as per original process_image_features

        # --- End of torch.no_grad() block ---

        # Update the class attribute self.features_valid if it was created (i.e., in paddle mode)
        if features_valid is not None:
            
            if self.features_valid is not None:
                # Now concatenate along batch dim if self.features_valid exists
                self.features_valid = torch.cat([self.features_valid, features_valid], dim=0)
            else:
                # This branch is hit on the first item of a batch if in paddle mode
                self.features_valid = features_valid

        # Return the main feature tensor
    ...
    def forward_one(self, batch, return_info=False):
        ...
        visual_features_valid = self.features_valid
    ...
    def forward_one_ce(self, batch, visual_features, visual_features_valid, return_info):
        ...
        if self.training:
            ...
            # print(caption_token_input.shape)
    ...
    def get_streaming_features(self, features, streaming_buffer_size, num_iters=10):
        # print("features.shape:", features.shape)
        # print("streaming_buffer_size:", streaming_buffer_size)
        centers=  features[:streaming_buffer_size]
        weights = torch.ones((features.shape[0],), dtype=torch.float32)
        feature = self.kmeans(centers, features, weights, num_iters=num_iters)[0]
        return feature

    def kmeans(self, init_centers, data, weights, num_iters=1):
        centers = init_centers.clone()  # (k, p, d)


        # if torch.isnan(data).any():
        #     print("data has NaN!")
        #     data = torch.nan_to_num(data, nan=0.0, posinf=1e8, neginf=-1e8)

        # if torch.isnan(centers).any():
        #     print("centers has NaN!")
        #     centers = torch.nan_to_num(centers, nan=0.0, posinf=1e8, neginf=-1e8)
        ...
        for _ in range(num_iters):
            ...
            #     # 找出哪幾行有 NaN
            #     nan_rows = torch.isnan(distances).any(dim=1).nonzero()
                # print(f"[iter={_}] distances 有 NaN, 行索引: {nan_rows}")
            #     # 您可以視需求做： distances = torch.nan_to_num(distances, ...)

            # if torch.isinf(distances).any():
            #     inf_rows = torch.isinf(distances).any(dim=1).nonzero()
            #     print(f"[iter={_}] distances 有 Inf, 行索引: {inf_rows}")
            assignments = torch.argmin(distances, dim=1)  # (n,)
            # print("assignments min:", assignments.min().item(), "max:", assignments.max().item(), "k:", k)

            # 計算加權的 data
            weighted_data = data * weights.unsqueeze(1)  # (n, p * d)

            # One-hot 編碼 assignments
            one_hot_assignments = F.one_hot(assignments, num_classes=k).type(dtype)  # (n, k)

            # 計算新的 centers
            weighted_sums = one_hot_assignments.t() @ weighted_data  # (k, p * d)
            counts = one_hot_assignments.t() @ weights  # (k,)
            counts_expanded = counts.unsqueeze(1) + 1e-8  # (k, 1)
            new_centers  = weighted_sums / counts_expanded  # (k, p * d)

            # 處理空的聚類中心，保留原始中心
            empty_clusters = (counts == 0).unsqueeze(1).expand_as(new_centers)  # (k, d)
            new_centers = torch.where(empty_clusters, centers, new_centers)
