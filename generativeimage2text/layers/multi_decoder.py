...
class CaptioningModel(nn.Module):
    def __init__(
        ...
        if context_not_share_embedding:
            import copy
            self.text_encoder = copy.deepcopy(self.textual.embedding)
    ...
    def forward_one(self, batch, return_info=False):
        ...
        if 'image' in batch:
            ...
            # print(f"Image features shape: {features[0].shape}")
        ...
        # 直接 stack 再轉置，而不做 pooling
        stacked_features = torch.stack(features, dim=0)  # (n, B, p, d)
        visual_features_4d = stacked_features.transpose(0, 1)  # (B, n, p, d)

        B, n, p, d = visual_features_4d.shape
        
        if "ocr_tokens" in batch:

            # batch['context'] (batch, n, q)

            # 選擇使用的 embedding
            context_embedding = (
                self.text_encoder if self.context_not_share_embedding else self.textual.embedding
            )
            
            # 我們已經有 (B, n, p, d) 在 visual_features_4d 中，方便後續依照每張圖插入文字
            # visual_features_4d.shape = (B, n, p, d)
            final_features_list = []
            final_valid_list = []

            # 假設 batch['context'] 的結構是 (B,) ，其中 batch['context'][b] = [text_for_img0, text_for_img1, ...]
            # 表示第 b 個 sample 對應 n 張圖的文本 tokens。
            ...
            for b_idx in range(B):
                ...
                texts_for_sample = batch['ocr_tokens'][b_idx]  # list of text tokens for each of the n images

                for i in range(n):
                    # 取出第 i 張圖片對應的 (p, d) 特徵，並擴張一個 batch 維度到 (1, p, d)
                    vis_i = visual_features_4d[b_idx, i]  # (p, d)
                    vis_i = vis_i.unsqueeze(0)            # (1, p, d)

                    # 計算視覺特徵的 valid mask
                    valid_vis_i = convert2valid(vis_i.shape[:2])  # (1, p)

                    sample_chunks.append(vis_i)
                    sample_valids.append(valid_vis_i)

                    # 文字部分: OCR / token
                    text_tokens = texts_for_sample[i]  # e.g. shape: (q,) with token IDs

                    # 尋找最後一個 [SEP]，若無則全部都是有效
                    sep_positions = torch.where(text_tokens == SEP_ID)[0]
                    if len(sep_positions) > 0:
                        text_length = sep_positions[-1].item() + 1
                    else:
                        text_length = text_tokens.size(0)

                    # 進行 embedding，維度會是 (1, text_length, d)
                    context_feat = context_embedding(text_tokens.unsqueeze(0))
                    # 建立文字的 valid mask
                    valid_text_i = convert2valid(text_tokens.unsqueeze(0).shape, torch.tensor([text_length], device=text_tokens.device)) 

                    sample_chunks.append(context_feat)
                    sample_valids.append(valid_text_i)

                # 對該 sample 將所有 chunks 在序列維度 (dim=1) 串接
                sample_feature = torch.cat(sample_chunks, dim=1)  # (1, total_seq_len, d)
                sample_valid = torch.cat(sample_valids, dim=1)    # (1, total_seq_len)

                final_features_list.append(sample_feature)
                final_valid_list.append(sample_valid)

            # 最後在 batch 維度 (dim=0) 上把所有 sample 串接
            visual_features = torch.cat(final_features_list, dim=0)      # (B, total_seq_len, d)
            visual_features_valid = torch.cat(final_valid_list, dim=0)   # (B, total_seq_len)

        print(f"visual_features shape: {visual_features.shape}")
        print(f"visual_features_valid shape: {visual_features_valid.shape}")
