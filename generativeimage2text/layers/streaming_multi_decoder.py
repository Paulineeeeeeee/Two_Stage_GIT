...
class CaptioningModel(nn.Module):
    ...
    def process_image_features(self, image):
        # image: (length, channels, height, width)

        with torch.no_grad():
            image = image.to("cuda")
            features = self.image_encoder(image)
            token_size = features.shape[1]
            # print(f"features_len:{len(features)}, features:{features[0].shape}")
            
        # if self.num_image_with_embedding and not self.streaming:
        if self.num_image_with_embedding:
            with torch.no_grad():
                # print('add embedding')
                # if length is not equal to num_image_with_embedding[0], interpolation
                if len(features) != self.num_image_with_embedding:
                    temp_emb_tensor = torch.stack(list(self.img_temperal_embedding), dim=0)  # shape: (8, 1, 1 ,token_size)
                    temp_emb_tensor = temp_emb_tensor.squeeze(2) 
                    temp_emb_tensor = temp_emb_tensor.permute(1, 2, 0)  # shape: (1, token_size, 8)
                    temp_emb_tensor = F.interpolate(temp_emb_tensor, size=len(features), mode='linear', align_corners=False)  # shape: (1, token_size, length)
                    temp_emb_tensor = temp_emb_tensor.permute(2, 0, 1)  # shape: (length, 1, token_size)
                    features = features + temp_emb_tensor
        ...
        if self.pooling_images is None:
            ...
            # visual_features = torch.cat(features, dim=0)
        elif self.pooling_images == 'avg':
            visual_features = torch.stack(visual_features, dim=1).mean(dim=1)
        else:
            raise NotImplementedError
        if self.streaming:
            visual_features = self.get_streaming_features(visual_features, self.num_image_with_embedding * token_size)
    ...
    def forward_one(self, batch, return_info=False):
        ...
        # features = [self.image_encoder(im) for im in batch['image']]  # List of tensors, each of shape (batch_size, p, d)
        
        # # print(f"features_len:{len(features)}, features:{features[0].shape}")
        # features = batch['image']
        # token_size = features[0].shape[1]

        # if self.num_image_with_embedding and not self.streaming:
        #     # print('add embedding')
        #     self.img_temperal_embedding.to(features[0].device)
        #     features = [f + e for f, e in zip(features, self.img_temperal_embedding)]
        #     # print(f"combine_feature_len:{len(combined_features)}, combined_features:{combined_features[0].shape}")
        # if self.pooling_images is None:
        #     # print('pooling') go here
        ...
        #         features = self.get_streaming_features(features, self.num_image_with_embedding * token_size)

        visual_features = batch['image']
        # print(visual_features.device)
    ...
    def get_streaming_features(self, features, streaming_buffer_size, num_iters=2):
