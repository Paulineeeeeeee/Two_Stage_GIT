...
class CaptioningModel(nn.Module):
    ...
    def forward_one(self, batch, return_info=False):
        ...
        if 'image' in batch:
            ...
            if self.pooling_images is None:
                ...
                # print(f"visual_feature:{visual_features.shape}")
    ...
    def get_visual_features(self, images):
        with torch.no_grad():
            features = self.image_encoder(images)  # (batch_size * num_images, p, d)
            if isinstance(features, tuple):
                features = features[0]  # 根據 image_encoder 的實現
            features = features.float()
        return features  # (batch_size * num_images, p, d)
